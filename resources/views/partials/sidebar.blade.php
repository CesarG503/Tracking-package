{{-- Sidebar Component --}}
<aside class="w-20 glass-sidebar flex flex-col items-center py-6 gap-2 border-r border-border">
    {{-- Logo --}}
    ...
    <div class="flex flex-col gap-2">
        <form action="{{ route('logout') }}" method="POST" id="sidebar-logout-form">
            @csrf
            <button type="button" onclick="confirmSidebarLogout()" class="w-12 h-12 rounded-xl text-foreground-muted flex items-center justify-center hover:bg-danger-light hover:text-danger transition-colors" title="Cerrar Sesion">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
