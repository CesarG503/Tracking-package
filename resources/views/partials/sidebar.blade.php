...
<aside class="w-20 bg-surface dark:glass-sidebar-dark flex flex-col items-center py-6 gap-2 border-r border-border dark:border-border transition-colors duration-300">
    ...
    <nav class="flex-1 flex flex-col gap-2">
        ...
        <a href="{{ route('envios.index') }}" class="w-12 h-12 rounded-xl {{ request()->routeIs('envios.*') ? 'bg-primary/10 text-primary dark:bg-primary/20 dark:text-primary' : 'text-foreground-muted hover:bg-surface-secondary dark:hover:bg-surface-secondary hover:text-foreground dark:hover:text-foreground' }} flex items-center justify-center transition-colors" title="Envios">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
            </svg>
        </a>
